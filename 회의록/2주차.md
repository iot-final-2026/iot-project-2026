...
- `started_at`
  - 실제 생산 세션을 시작한 시각
- `created_at`
  - 생산 세션 데이터가 DB에 처음 생성된 시각
  - 관리자가 작업 계획을 미리 등록하는 경우 사용 가능

#### 확인 사항

- 관리자가 작업 계획을 미리 등록할 것인지 확인
- `started_at`과 `created_at`을 모두 유지할 필요가 있는지 검토
...
- 알림에 기록되는 사용자가 오류 발생 당시 작업자인지, 관리자인지 구분
- 해당 사용자 정보를 `ALERTS` 테이블에 직접 기록할 것인지 검토
...
```text
시작 → 로그인 → 안내문 → 작업자 메인 화면 → 종료 및 로그아웃
```
